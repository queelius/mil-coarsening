...
here is one case. The nearest existing MIL-identifiability statement is
the ``true MIL'' analysis of \citet{doran2014theoretical}, who
characterize when the bag-formation geometry admits a consistent
instance-level classifier; that is a well-posedness condition on the
data design, whereas our rank condition is a parameter-level statement
about which instance-type positivities are recoverable from the bag
likelihood, so the two are complementary objects rather than competitors.

\subsection{The bridge to masked-data inference}

We observe that MIL is an instance of the masked-data
...
\item \textbf{A bias bound under aggregation-rule misspecification}
  ...
  \citet{foulds2010review} is recovered as a classification of how
  aggregation rules depart from the single-cause OR structure:
  threshold and proportion rules preserve the C1 support condition,
  while emergent rules violate it.
