...
injective. The bag labels then form a Bernoulli GLM with the
log-survival link $g(\mu) = -\log(1-\mu)$ and design $M$; the linear
predictor, hence $\bm{s}$,
...
\begin{remark}
The bag-OR model $p = 1 - e^{-\bm m^\top \bm s}$ uses the
log-survival (negative-log-complement) link $g(\mu) = -\log(1-\mu)$,
which is linear in $\bm s$. This is \emph{not} the canonical link
for the Bernoulli family (logit is), nor is it the
complementary-log-log link $\log(-\log(1-\mu))$; it is the link that
makes the bag-level log-survival additive in the instance
contributions. The unweighted moment identity $M^\top(\bm Y -
\hat{\bm p}) = \bm 0$ holds for the canonical logit GLM but not for
this model. The IRLS weights $1/\hat p_i$ in \eqref{eq:bag-total}
are exactly the variance-link factors arising from the
non-canonical link, and cannot be removed by reparametrization
...
mis-ranking. This gives a likelihood-level mechanism for the
empirical bag-versus-instance accuracy gap documented by
\citet{vanwinckelen2016instance}: where they observe that bag-level
performance need not transfer to instances, \eqref{eq:bag-total}
identifies the rank-deficient directions of $M$ as exactly the
subspace in which instance mass is unconstrained by the bag
likelihood. Diagnosing per-instance bias requires instance-level
