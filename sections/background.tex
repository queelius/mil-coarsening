...
with this work or with the companion applications
...
survival function. The third factor is the unknown masking
mechanism.

These conditions are the masked-cause reading of the general
coarse-data theory of \citet{heitjan1991ignorability}, applied to
weak supervision and set-valued (superset) labels by
\citet{couso2017coarse}; the present paper specializes that coarse-data
maximum-likelihood machinery to the series-system (logical-OR) channel.
...
Under all three, the likelihood collapses to a product of a
system-level factor and a candidate-set sum that no longer involves
the masking mechanism, and \citet[Theorem 8]{towell2026masked}
...
\in \{0,1\}^{n \times m}$ is \emph{sufficient} for $\theta$ to be
...
isolates a single component contribution and adds an identifying
unit-vector row to $\tilde{C}$.

The framework has been ported to scRNA-seq zero inflation, where the
candidate set per gene is a single bit (zero vs. nonzero count)
\citep{towell2026scrnacoarsening}, and to spatial-transcriptomics
deconvolution, where each spot carries a probability distribution
over cell types \citep{towell2026spatialcoarsening}. We now port it
to multiple instance learning, where the latent quantity and the
observed label are both \emph{discrete}: the candidate set is a bag
of instances, and the coarsening is logical-OR aggregation of binary
instance labels.
