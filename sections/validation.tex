...
practical reason the cloglog-link non-canonicity in the remark
...
\paragraph{Bag-prevalence consistency, real data.} The fitted
bag-positive rate $\bar{\hat p}$ matches the observed bag-positive
rate $\bar Y$ to $0.03$ on MUSK1 ($\bar{\hat p} = 0.49$ vs.
$\bar Y = 0.51$) and to $0.13$ on MUSK2 ($0.25$ vs. $0.38$). The
MUSK2 gap is the boundary-coordinate signature of
\cref{thm:bag-total}: at the KKT optimum the column-space moment
identity holds for interior coordinates, while the constrained
coordinates carry a residual that the unweighted bag rate
inherits.
...
confound; (ii) the bag-prevalence consistency theorem produces the
observed bag-rate to within the boundary-coordinate residual the
theorem itself predicts; (iii) the interior-versus-boundary MLE
