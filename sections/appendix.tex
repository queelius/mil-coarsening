...
\subsection{Proof of Theorem 1 (rank condition)}
...
The proof reveals that the identifying structure is the cloglog
...
\subsection{Proof of Theorem 4 (continuous-feature rank condition)}
...
\subsection{Proof of Theorem 2 (bag-prevalence consistency)}
...
\subsection{Proof of Theorem 3 (aggregation-rule bias)}
...
+ O(\epsilon^2)$
with $\dot s_k = -(e^{s_k^*} - 1)$ and
$p_i^* = 1 - e^{-\bm m_i^\top \bm s^*}$. The score sensitivity is
\begin{equation}
...
\cref{fig:firing-rate}: the bias rotates $\hat{\bm s}$ along the
direction $\dot{\bm s}$ that corresponds to a multiplicative
rescaling of $\bm\eta$ by $\rho$.
