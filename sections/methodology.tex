...
the proportion or count of positive instances. In our framework these
departures from deterministic OR fall into two tiers. The standard
assumption is the regime in which the single-cause OR structure holds:
a positive bag is positive by virtue of one individually positive
instance, and C1 holds with probability one. Threshold, count, and
proportion rules \emph{preserve} C1, since a positive bag still
contains individually positive instances, but they break the
single-cause structure: the masked ``cause'' becomes a subset of
instances rather than a single one, so the deterministic-OR likelihood
is misspecified even though its support condition is intact. Emergent
(relational) rules, in which bag positivity is a collective property
with no individually positive instance, \emph{violate} C1 outright.
\Cref{thm:bias-rule} covers both tiers: it bounds the bias of the
deterministic-OR estimator under any smooth departure $\bm{\epsilon}$
from the OR rule, first-order in $\bm{\epsilon}$ and zero precisely at
$\bm{\epsilon} = \bm{0}$, whether that departure preserves C1
(threshold, count, proportion) or violates it (emergent). The
...
OR structure, and the bias bound quantifies the cost of each
departure.
